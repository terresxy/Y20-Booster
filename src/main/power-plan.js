...
const BASE_RELATIVE_PATH = ['Imagens', 'OptimizationPack', 'PLANOS DE ENERGIA'];
...
async function collectEntries(dirPath) {
  ...
  for (const name of listing) {
    ...
    // Ignorar subpastas (como "Remover Plano energia")
...
async function removeAllPowerPlans() {
  const baseDir = resolveBaseDir();
  const removeFolderPath = path.join(baseDir, 'Remover Plano energia');
  
  try {
    await fs.access(removeFolderPath);
  } catch (error) {
    throw new Error('Pasta "Remover Plano energia" não encontrada.');
  }

  // Procurar arquivo .bat na pasta
  let files = [];
  try {
    files = await fs.readdir(removeFolderPath);
  } catch (error) {
    throw new Error('Erro ao ler pasta de remoção.');
  }

  const batFile = files.find(f => f.toLowerCase().endsWith('.bat'));
  
  if (!batFile) {
    throw new Error('Script de remoção não encontrado.');
  }

  const scriptPath = path.join(removeFolderPath, batFile);
  
  if (!fssync.existsSync(scriptPath)) {
    throw new Error('Script de remoção não encontrado.');
  }

  const preparedPath = await prepareExecutable(scriptPath);
...
async function prepareExecutable(sourcePath) {
  ...
  const tempDir = path.join(app.getPath('temp'), 'y20-power-plan');
  ...
  return new Promise((resolve, reject) => {
    ...
    let timeoutHandle = setTimeout(() => {
      if (child && child.exitCode === null) {
        console.error(`[PowerPlan] executeProcess: Timeout de ${timeoutMs}ms atingido para ${command}. Encerrando processo...`);
        try {
          child.kill('SIGTERM');
          setTimeout(() => {
            if (child && child.exitCode === null) {
          ...
          console.error(`[PowerPlan] executeProcess: Erro ao encerrar processo:`, error);
        }
